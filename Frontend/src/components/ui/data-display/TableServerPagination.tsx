...
function TableServerPagination<T extends Record<string, any>>({
    ...
    const displayData = useMemo(
        () => (preserveDataWhileLoading && cachedData.length > 0 ? cachedData : data ?? []),
                        ...
                        <tr>
                            {columns.map((column, index) => {
                                ...
                                const arrow = !isSortable ? null : (
                                    <span
                                        className={`ml-1 inline-block transition-transform ${isActive && sortOrder === 'desc' ? 'rotate-180' : ''
                                            }`}
                                    >
                                        <svg
                                ...
                                return (
                                    <th
                                        key={index}
                                        className={`px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider ${column.align === 'center'
                                            ? 'text-center'
                                            : column.align === 'right'
                                                ? 'text-right'
                                                : 'text-left'
